...
OPENROUTER_URL = os.getenv("URL")
MODEL = os.getenv("MODEL_ID")
DB_URL = os.getenv("DB")
PER_DIR = os.getenv("PERSISTENT_DIR")
COLLECTION_NAME = os.getenv("COLLECTION_NAME")
BM25_DIR = os.getenv("BM_25_DIR")
...
vector_store = Chroma(
    ...
    collection_name=COLLECTION_NAME,  # Explicit collection name # type: ignore
)
...
# Load BM25 retriever
with open(BM25_DIR, "rb") as f:  # type: ignore
...
# ============================================================================
RAG_PROMPT_TEMPLATE = """You are an expert AI assistant that answers questions using ONLY the provided context documents.
...
- Use lists (bullet points or numbered) where appropriate
...
class RatingScore(BaseModel):
    ...
    relevance_score: int = Field(
        ..., description="Relevance score (1-10) of document to query", ge=1, le=10
    )


# OPTIMIZATION 1: Fast RRF Reranking (No LLM calls - 10-50x faster)
def rerank_with_rrf(
    query: str, docs: List[Document], top_n: int = 10, k: int = 60
) -> List[Document]:
    """
...
def rerank_with_llm(
    query: str, docs: List[Document], top_n: int = 10
) -> List[Document]:
    """
    ...
    rerank_llm = LLMProvider.get_llm(
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for doc in docs:
            future = executor.submit(
                chain.invoke, {"query": query, "doc": doc.page_content[:500]}
            )
...
def create_advanced_rag_chain_with_expansion(
    llm_provider: str = "groq", use_query_expansion: bool = True, num_queries: int = 2
):
    """
...
def create_custom_rag_chain_with_expansion(
    ...
    if use_reranking:
        chain = (
            ...
            | RunnableLambda(
                lambda x: build_context_with_expansion(x, use_expansion=False)  # type: ignore
            )  # Expansion already done # type: ignore
            | prompt
            | current_llm
            | StrOutputParser()
        )
        ...
        chain = (
            {"context": retriever_step | format_docs, "input": RunnablePassthrough()}
            | prompt
            | current_llm
            | StrOutputParser()
        )
...
advanced_rag_chain = create_advanced_rag_chain_with_expansion(
    "groq", use_query_expansion=True, num_queries=2  # Enable query expansion by default
)
...
simple_rag_chain = create_simple_rag_chain_with_expansion(
    "groq", use_query_expansion=False  # Disabled for speed
)
...
def create_advanced_rag_chain(llm_provider: str = "groq"):
    ...
    return create_advanced_rag_chain_with_expansion(
        llm_provider, use_query_expansion=False
    )
...
def create_custom_rag_chain(llm_provider: str = "groq"):
    ...
    return create_custom_rag_chain_with_expansion(
        llm_provider, use_query_expansion=False, use_reranking=True
    )
...
Provide confidence score and reasoning:"""
    ...
    result = chain.invoke(
        {"input": question, "answer": answer, "context": context[:2000]}
    )
...
# ============================================================================
def expand_query(
    question: str, num_queries: int = 2, use_llm: bool = True
) -> List[str]:
    """
...
if __name__ == "__main__":
    ...
    gemini_chain = create_custom_rag_chain_with_expansion(
        "google_ai", use_query_expansion=True, use_reranking=True
    )
    start = time.time()
