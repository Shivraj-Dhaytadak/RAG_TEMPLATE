# main.py
# Enterprise-Grade HR Handbook Chatbot UI (LangChain Edition)
...
# Import from the LangChain chatbot
from chatbot_langchain import (
    PDF_DIR,
    MODELS_DIR,
...
# Note: chatbot_langchain uses LangChain Documents, not a custom Chunk class

# ==================== PAGE CONFIG ====================
st.set_page_config(
    page_title="HR Assistant (LangChain)",
    page_icon="💼",
    layout="wide",
...
def load_all_models():
    ...
    retriever_chain = setup_retrieval_chain(vector_store, bm25_retriever)
    ...
    rag_chain = create_chain(retriever_chain, llm)
...
def initialize_models():
    ...
    with loading_container.container():
        ...
        <div style='text-align: center; padding: 3rem;'>
            ...
            <p style='color: #64748b;'>Setting up your intelligent HR companion (LangChain Powered)...</p>
        </div>
        """,
            unsafe_allow_html=True,
        )
        ...
        try:
            status.markdown("**Loading knowledge base...**")
            ...
            status.markdown("**Configuring LLM & Chains...**")
...
# ==================== HELPER FUNCTIONS ====================
def format_single_source_html(doc: Any, index: int) -> str:
    """Format a single source as HTML (Adapted for LangChain Document)"""
    excerpt = doc.page_content.replace("\n", " ")
    # Truncate if too long for display
    if len(excerpt) > 500:
        excerpt = excerpt[:500] + "..."

    source_file = Path(doc.metadata.get("file_path", "unknown")).name
...
def get_answer(question: str) -> Tuple[str, float, List[Any]]:
    """Get answer from chatbot"""
    start_time = time.time()

    # 1. Retrieve first to share sources (optional, but good for UI)
    retriever_chain = st.session_state.retriever_chain
    selected_docs = retriever_chain.invoke(question)

    # 2. Generate Answer
    rag_chain = st.session_state.rag_chain
    # The chain expects `question` as input and handles retrieval internally if we used `create_retrieval_chain`,
    # BUT in chatbot_langchain we defined a custom chain:
    # chain = {"context": retriever | format_docs, "question": RunnablePassthrough()} | prompt | llm ...
    # So we just invoke it with the question string.

    # However, to avoid double retrieval (once for UI, once for chain),
    # we might want to pass pre-retrieved docs if the chain supported it.
    # Our current `chatbot_langchain.create_chain` wires retrieval directly into the chain.
    # To keep it simple and robust, we will just let the chain run (it rede-retrieves)
    # OR we rely on the `selected_docs` we just fetched and only use valid sources for UI.
    # For now, let's just let the chain run. It's fast enough.

    answer = rag_chain.invoke(question)

    elapsed = time.time() - start_time
    return answer, elapsed, selected_docs
...
def main():
    ...
    <div class="custom-header">
        <div class="header-title">
            💼 HR Assistant (LangChain)
        </div>
        <div class="header-subtitle">
            Your intelligent companion for HR policies and benefits (Powered by vLLM/LlamaCpp)
        </div>
    </div>
    """,
        unsafe_allow_html=True,
    ...
    with st.sidebar:
        st.markdown(
            '<div class="sidebar-header">💬 Conversations</div>', unsafe_allow_html=True
        )
        ...
        with st.expander("⚙️ Settings"):
            ...
            # Rebuild currently not fully supported via UI in main.py simple version because
            # create_or_load_index is inside cache_resource.
            st.info(
                "To rebuild index, restart app with --reindex flag or delete index folder manually."
            )
    ...
    for idx, message in enumerate(st.session_state.messages):
        with st.chat_message(
            message["role"], avatar="👤" if message["role"] == "user" else BOT_AVATAR
        ):
            ...
            if message["role"] == "assistant":
                if "time" in message:
                    st.markdown(
                        f'<span class="response-badge">⚡ {message["time"]:.1f}s</span>',
                        unsafe_allow_html=True,
                    )
                ...
                if (
                    ...
                    with st.expander(
                        f"📚 View {len(message['sources'])} Sources", expanded=False
                    ):
                        ...
                        for source_idx, source in enumerate(message["sources"], 1):
                            ...
                            source_title = f"📄 {source_file} - Page {page}"

                            with st.expander(source_title, expanded=False):
                                st.markdown(
                                    format_single_source_html(source, source_idx),
                                    unsafe_allow_html=True,
                                )
    ...
    if prompt := st.chat_input(
        "Ask me about HR policies, benefits, procedures...", key="chat_input"
    ):
        process_user_query(prompt)
...
def process_user_query(prompt: str):
    ...
    with st.chat_message("assistant", avatar=BOT_AVATAR):  # type: ignore
        ...
        try:
            ...
            st.markdown(answer)
            st.markdown(
                f'<span class="response-badge">⚡ {elapsed:.1f}s</span>',
                unsafe_allow_html=True,
            )
