...
class QueryRequest(BaseModel):
    ...
    include_confidence: Optional[bool] = Field(False, description="Include confidence score")
...
class IngestRequest(BaseModel):
    file_path: str
    chunking_strategy: Optional[str] = Field("recursive", description="Chunking strategy: recursive, semantic, parent_child")
    ...
    async def generate():
        try:
            ...
            async for chunk in stream_rag_response(
                request_data.question,
                request_data.llm_provider or "groq"
            ):
                # Send as SSE format
                yield f"data: {chunk}\n\n"
